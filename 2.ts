...
interface ITask {
  ...
  title: string;
  completed: boolean;
  priority?: 'high' | 'medium' | 'low'; // Опциональное поле, enum
}

// tasks - массив объектов типа ITask
const tasks: ITask[] = [
...
function filterCompletedTasks(tasks: ITask[]): ITask[] {
  return tasks.filter(task => task.completed); // через метод filter возращает только выполненые задачи
}
